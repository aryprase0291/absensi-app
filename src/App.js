...
const handleLogin = (userData, rawMasterData, versiServer, statsAwal, pengumumanAwal, pengumumanDisertakan, periodsAwal) => {
  ...
  useEffect(() => {
    ...
    // tetap berjalan saat karyawan berpindah-pindah layar. Dihentikan
    // otomatis saat user berubah/logout — kalau tidak, timernya akan
    // menembak memakai token milik user sebelumnya.
    // ----------------------------------------------------------------
useEffect(() => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (data.result === 'success') {
        ...
        // Titik absen ikut masuk ke pelacakan (GpsTracking.gs). Dikirim
        // TANPA await dan tanpa penanganan error: absen sudah tersimpan,
        // dan kegagalan mengirim satu titik pemantauan tidak boleh
        // mengubah apa pun yang dilihat karyawan.
        if (location && (type === 'Hadir' || type === 'Pulang' || type === 'Dinas Luar' || type === 'Dinas')) {
          kirimTitikGps({
            fetchApi, scriptUrl: SCRIPT_URL, user, sumber: 'absen',
            posisi: { lat: location.lat, lng: location.lng, akurasi: gpsValidation.accuracy }
          });
        }
